...
if (isset($_GET['s'])) {
  $st_code = mysqli_real_escape_string($con, $_GET['s']);
  $
}
if (isset($_GET['s'])) {
  $searchstring = mysqli_real_escape_string($con,$_GET['s']);
}
$sql = "SELECT * FROM `skilled_migrants` WHERE `st_code` = 1";
$rsJobs = mysqli_query($con,$sql) or die("Sorry, the server's too busy. Relax, take a deep breath, and refresh the page");
?>
<!DOCTYPE html>
<head lang="en-au">
    <meta charset="utf-8">
