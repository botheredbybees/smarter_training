...
require_once('Connections/tp.php');
  ...
  if (isset($_GET['s'])) {
    ...
    if (isset($_GET['d'])) { // we want the jobs in this district for this industry
      ...
      $sql = "SELECT  distinct occupation, occupation_name, st_name, `industry_name`  FROM `skilled_migrants` WHERE `st_code` = $state and sa2 = $d order by `industry_name`";    
    }
